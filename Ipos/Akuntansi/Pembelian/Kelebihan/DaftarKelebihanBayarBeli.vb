Public Class DaftarKelebihanBayarBeli
    ...
    Sub fillData()
        ...
        Dim sql As String = "select tglkelebihanbeli,kodekelebihanbayarbeli,tblkontak.pelanggan, tblkelebihanbayarbeli.kodebeli,tblkelebihanbayarbeli.jumlah as nilai from tblkelebihanbayarbeli inner join tblbeli on tblbeli.kodebeli = tblkelebihanbayarbeli.kodebeli inner join tblkontak on tblkontak.idpelanggan = tblbeli.pelanggan "
        Dim filter As String = "WHERE (kodekelebihanbayarbeli ilike '%" & cari & "%' or tblkontak.pelanggan ilike '%" & cari & "%') AND tglkelebihanbeli BETWEEN '" & tglAwal & "' AND '" & tglAkhir & "' order by tglkelebihanbeli,kodekelebihanbayarbeli"
    ...
    Sub tambahData()
        Dim ds As DialogResult = FormKelebihanBayarBeli.ShowDialog()
        If ds = DialogResult.OK Then

        End If
        fillData()
        FormKelebihanBayarBeli.Dispose()
    End Sub
        ...
        If ListSat.SelectedRows.Count = 1 Then
            ...
            If True Then
                If dialog("Apakah anda yakin untuk menghapus data ini ?") Then
                    Dim sqlhapus = "DELETE FROM tblkelebihanbayarbeli where kodekelebihanbayarbeli = '" & idselected & "';"
