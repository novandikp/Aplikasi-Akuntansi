Public Class DaftarPesananBeli
    ...
    Sub fillData()
        ...
        Dim sql As String = "SELECT tglpesananbeli,kodepesananbeli, tblkontak.pelanggan, total,diskonrupiah,biayalain,totalpajak,cast(statuspesanan as varchar) from tblpesananbeli inner join tblkontak on tblkontak.idpelanggan = tblpesananbeli.pelanggan "
        Dim filter As String = "WHERE (kodepesananbeli ilike '%" & cari & "%' or tblkontak.pelanggan ilike '%" & cari & "%') AND tglpesananbeli BETWEEN '" & tglAwal & "' AND '" & tglAkhir & "' order by tglpesananbeli,kodepesananbeli"
    ...
    Sub tambahData()
        Dim ds As DialogResult = FormPesananbeli.ShowDialog()
        If ds = DialogResult.OK Then

        End If
        fillData()
        FormPesananbeli.Dispose()
    End Sub
    ...
    Sub editData()
        If ListSat.SelectedRows.Count = 1 Then
            ...
            Dim pesanan As New FormPesananbeli
    ...
    Sub hapusData()
        If ListSat.SelectedRows.Count = 1 Then
            Dim idselected As String = ListSat.Rows(ListSat.SelectedRows(0).Index).Cells(1).Value
            Dim sqlparent As String = "SELECT kodepesananbeli from tblpengirimanbeli where kodepesananbeli='" & idselected & "'"
            Dim sqlparent1 As String = "SELECT kodepesananbeli from tblbeli where kodepesananbeli='" & idselected & "'"
            If getCount(sqlparent) = 0 And getCount(sqlparent1) = 0 Then
                If dialog("Apakah anda yakin untuk menghapus data ini ?") Then
                    Dim sqlhapus = "DELETE FROM tblpesananbeli where kodepesananbeli = '" & idselected & "';"
                    Dim sqlhapusdetail = "DELETE FROM tbldetailpesananbeli where kodepesananbeli = '" & idselected & "';"
