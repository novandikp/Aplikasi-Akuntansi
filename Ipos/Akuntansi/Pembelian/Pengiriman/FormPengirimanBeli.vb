...
Public Class FormPengirimanBeli
    ...
    Sub cariPenawaranbeli()
        If Not edited Then
            Dim dialog As New DialogPesananbeli
            If dialog.ShowDialog = DialogResult.OK Then
                tbPenawaranbeli.Text = dialog.kodepesananbeli
    ...
    Sub awal()
        ...
        Else
            ...
            TBnotransaksi.Text = generateRefrence("BD")
    ...
    Sub continueOrder()
        Dim sqlorder As String = "select kodepengirimanbeli,kodepesananbeli, tglpengirimanbeli, kodedepartemen, kodeprojek, total, diskonrupiah, diskonpersen, totalpajak, biayalain, kasbiayalain, kasdiskon, nomerdokumen, tgldokumen, pelanggan, kodegudang from tblpengirimanbeli where kodepengirimanbeli='" & TBnotransaksi.Text & "'"
        Dim sqldetail As String = "select tbldetailpengirimanbeli.idharga,diskondetailpersen,idproduk,nilaidasar,produk, idbarang,jumlahbeli, tbldetailpengirimanbeli.hargabeli,satuan,jumlahpajak from  tbldetailpengirimanbeli  inner join tblharga on tblharga.idharga = tbldetailpengirimanbeli.idharga inner join tblproduk on tblproduk.idproduk = tblharga.idbarang inner join tblsatuan on tblsatuan.kodesatuan = tblharga.idsatuan where kodepengirimanbeli='" & TBnotransaksi.Text & "'"
        saved = True
        dgKeranjang.Rows.Clear()
        CBsupplier.SelectedValue = getValue(sqlorder, "pelanggan")
        cbGudang.SelectedValue = getValue(sqlorder, "kodegudang")
        cbProjek.SelectedValue = getValue(sqlorder, "kodeprojek")
        tbPenawaranbeli.Text = getValue(sqlorder, "kodepesananbeli")
        Dim tableExist As DataTable = getData(sqldetail)
        Dim no = 0
        For Each row As DataRow In tableExist.Rows
            dgKeranjang.Rows.Add()
...
GROUP by  tblharga.idbarang,T.nilaidasar, B.nilaidasar,T.idharga,B.idharga ,Y.idharga "
            ...
            For Each rowStok As DataRow In dt.Rows
                ...
                If edited And rowStok.Item("stok") < 0 Then
                    Dim sqlCheckOldDetail = "SELECT COALESCE(sum(jumlahbeli*nilaidasar),0) as total FROM tbldetailpengirimanbeli inner join tblharga on tblharga.idharga = tbldetailpengirimanbeli.idharga WHERE tbldetailpengirimanbeli.kodepengirimanbeli = '" & TBnotransaksi.Text & "' and idbarang ='" & rowStok.Item("idbarang") & "'"
    ...
    Sub simpanBayar(bayar As DialogTransaksiTanpaBayar)
        ...
        If cekKeranjang(True) Then
            If Not edited Then
                ...
                Dim sql As String = "INSERT INTO public.tblpengirimanbeli(kodepengirimanbeli, tglpengirimanbeli, kodedepartemen, kodeprojek, total, diskonrupiah, diskonpersen, totalpajak, biayalain, kasbiayalain, kasdiskon, nomerdokumen, tgldokumen, pelanggan, kodegudang, statuspengiriman,kodepesananbeli) 	VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?);"
    ...
    Sub isidetail(refrensi As String)
        ...
        For Each row As DataGridViewRow In dgKeranjang.Rows
            If Not IsNothing(row.Cells(1).Value) Then
                ...
                Dim sqldetail As String = "INSERT INTO public.tbldetailpengirimanbeli(kodepengirimanbeli, jumlahbeli, hargabeli, jumlahpajak, catatandetail, idharga,diskondetailpersen)	VALUES ( ?, ?, ?, ?, ?, ?,?);"
        ...
        Dim akunPersediaanBelumDitagihkan = "149001"

        Dim dataDebit As String() = {akunPersediaanBelumDitagihkan, kodeprojek, ComboBox1.SelectedValue, CBsupplier.SelectedValue, dtTanggal.Value.ToString("yyyy-MM-dd HH:mm:ss").Replace(".", ":"), totalPembelian, "0", "BD", TBnotransaksi.Text, "Pengiriman Pembelian, " & CBsupplier.Text}
        Dim dataKredit As String() = {akunPersediaan, kodeprojek, ComboBox1.SelectedValue, CBsupplier.SelectedValue, dtTanggal.Value.ToString("yyyy-MM-dd HH:mm:ss").Replace(".", ":"), "0", totalPembelian, "BD", TBnotransaksi.Text, "Pengiriman Pembelian, " & CBsupplier.Text}
...
dialog.klasifikasiPotonganHarga="5100"
        ...
        If edited Then
            ...
            dialog.keyRefrensi = "kodepengirimanbeli"
