...
Public Class FormPenawaranBeli
    ...
    'cari pelanggan
    Sub cariSupplier()
        Dim dialogSupplier As New DialogKontak
        dialogSupplier.posisi = "Supplier"
        If dialogSupplier.ShowDialog = DialogResult.OK Then
            CBsupplier.SelectedValue = dialogSupplier.idpelanggan
        End If
        dialogSupplier.Dispose()
    End Sub
    'set projek
    Sub setProjek()
    ...
    Sub continueOrder()
        Dim sqlorder As String = "select kodepenawaranbeli, tglpenawaranbeli, kodedepartemen, kodeprojek, total, diskonrupiah, diskonpersen, totalpajak, biayalain, kasbiayalain, kasdiskon, nomerdokumen, tgldokumen, pelanggan, kodegudang from tblpenawaranbeli where kodepenawaranbeli='" & TBnotransaksi.Text & "'"
        Dim sqldetail As String = "select tbldetailpenawaranbeli.idharga,idproduk,diskondetailpersen,produk, idbarang,jumlahbeli, tbldetailpenawaranbeli.hargabeli,satuan,jumlahpajak from  tbldetailpenawaranbeli  inner join tblharga on tblharga.idharga = tbldetailpenawaranbeli.idharga inner join tblproduk on tblproduk.idproduk = tblharga.idbarang inner join tblsatuan on tblsatuan.kodesatuan = tblharga.idsatuan where kodepenawaranbeli='" & TBnotransaksi.Text & "'"
        saved = True
        dgKeranjang.Rows.Clear()
    ...
    Sub simpanBayar(bayar As DialogTransaksiTanpaBayar)
        ...
        If Not edited Then
            ...
            Dim sql As String = "INSERT INTO public.tblpenawaranbeli(kodepenawaranbeli, tglpenawaranbeli, kodedepartemen, kodeprojek, total, diskonrupiah, diskonpersen, totalpajak, biayalain, kasbiayalain, kasdiskon, nomerdokumen, tgldokumen, pelanggan, kodegudang, statuspenawaran) 	VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);"
        ...
        Else
            ...
            Dim sql As String = "UPDATE public.tblpenawaranbeli SET kodepenawaranbeli=?, tglpenawaranbeli=?, kodedepartemen=?, kodeprojek=?, total=?, diskonrupiah=?, diskonpersen=?, totalpajak=?, biayalain=?, kasbiayalain=?, kasdiskon=?, nomerdokumen=?, tgldokumen=?, pelanggan=?, kodegudang=? where kodepenawaranbeli=?;DELETE FROM tbldetailpenawaranbeli where kodepenawaranbeli=?"
            If operationQuery(sql, isidata) Then
                isidetail(TBnotransaksi.Text)
                dialogSukses("Berhasil")
                edited = False
    ...
    Sub isidetail(refrensi As String)
        For Each row As DataGridViewRow In dgKeranjang.Rows
            If Not IsNothing(row.Cells(1).Value) Then
                ...
                Dim sqldetail As String = "INSERT INTO public.tbldetailpenawaranbeli(kodepenawaranbeli, jumlahbeli, hargabeli, jumlahpajak, catatandetail, idharga,diskondetailpersen)	VALUES ( ?, ?, ?, ?, ?, ?,?);"
