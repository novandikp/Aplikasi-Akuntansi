Public Class DaftarPenawaranBeli
    ...
    Sub fillData()
        ...
        Dim sql As String = "SELECT tglpenawaranbeli,kodepenawaranbeli, tblkontak.pelanggan, total,diskonpersen,biayalain,totalpajak,cast(statuspenawaran as varchar) from tblpenawaranbeli inner join tblkontak on tblkontak.idpelanggan = tblpenawaranbeli.pelanggan "
        Dim filter As String = "WHERE (kodepenawaranbeli ilike '%" & cari & "%' or tblkontak.pelanggan ilike '%" & cari & "%') AND tglpenawaranbeli BETWEEN '" & tglAwal & "' AND '" & tglAkhir & "' order by tglpenawaranbeli,kodepenawaranbeli"
    ...
    Sub tambahData()
        Dim ds As DialogResult = FormPenawaranbeli.ShowDialog()
        If ds = DialogResult.OK Then

        End If
        fillData()
        FormPenawaranbeli.Dispose()
    End Sub
    ...
    Sub editData()
        If ListSat.SelectedRows.Count = 1 Then
            ...
            Dim penawaran As New FormPenawaranbeli
    ...
    Sub hapusData()
        If ListSat.SelectedRows.Count = 1 Then
            Dim idselected As String = ListSat.Rows(ListSat.SelectedRows(0).Index).Cells(1).Value
            Dim sqlparent As String = "SELECT kodepenawaranbeli from tblpesananbeli where kodepenawaranbeli='" & idselected & "'"
            If getCount(sqlparent) = 0 Then
                If dialog("Apakah anda yakin untuk menghapus data ini ?") Then
                    Dim sqlhapus = "DELETE FROM tblpenawaranbeli where kodepenawaranbeli = '" & idselected & "';"
                    Dim sqlhapusdetail = "DELETE FROM tbldetailpenawaranbeli where kodepenawaranbeli = '" & idselected & "';"
