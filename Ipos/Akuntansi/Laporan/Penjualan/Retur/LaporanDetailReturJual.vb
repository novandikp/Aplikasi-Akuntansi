Public Class LaporanDetailReturJual
    ...
    Sub getDataLaporan()
        ...
        If cbSub.SelectedIndex > 0 Then
            sql = "SELECT  tbldetailreturjual.kodereturjual as kode,tglreturjual as tgl,produk, jumlahjual as jumlah,satuan, tbldetailreturjual.hargajual as harga, jumlahpajak, diskondetailpersen, tblkontak.pelanggan as kontak from tbldetailreturjual inner join tblreturjual on tblreturjual.kodereturjual = tbldetailreturjual.kodereturjual inner join tblharga on tblharga.idharga = tbldetailreturjual.idharga inner join tblsatuan on tblsatuan.kodesatuan = tblharga.idsatuan inner join tblproduk on tblproduk.idproduk = tblharga.idbarang inner join tblkontak on tblkontak.idpelanggan = tblreturjual.pelanggan
WHERE (tblreturjual.kodereturjual ILIKE '%" & eCari.Text & "%' or tblkontak.pelanggan ILIKE '%" & eCari.Text & "%'  )
...
WHERE (tblreturjual.kodereturjual ILIKE '%" & eCari.Text & "%' or tblkontak.pelanggan ILIKE '%" & eCari.Text & "%'  )
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        PreviewReturJual.dataview = dv
        PreviewReturJual.detail = True
        PreviewReturJual.Show()
    End Sub
