    ...
    Sub getDataLaporan()
        If ringkasan Then
            ...
            If cbSub.SelectedIndex > 0 Then
                sql = "SELECT tglreturjual as tglpenawaran,  kodereturjual as kodepenawaranjual, tblkontak.pelanggan as idkontak,total, totalpajak
FROM tblreturjual
INNER join tblkontak
on tblkontak.idpelanggan = tblreturjual.pelanggan
        WHERE (kodereturjual ILIKE '%" & eCari.Text & "%' or tblkontak.pelanggan ILIKE '%" & eCari.Text & "%'  )
...
on tblkontak.idpelanggan = tblreturjual.pelanggan
        ...
        Else
            If cbSub.SelectedIndex > 0 Then
                sql = "SELECT tglreturjual as tglpenawaran, kodereturjual as kodepenawaranjual, tbldepartemen.departemen as kodedepartemen, COALESCE(kodeprojek,'N\A') as kodeprojek,total, diskonrupiah, diskonpersen, totalpajak, biayalain
FROM tblreturjual
INNER join tbldepartemen
on tbldepartemen.iddepartemen = tblreturjual.kodedepartemen
WHERE (kodereturjual ILIKE '%" & eCari.Text & "%' or departemen ILIKE '%" & eCari.Text & "%'  )
          and kodedepartemen ='" & cbSub.SelectedValue & "' and tglreturjual BETWEEN '" & dtAwal.Value.ToString("yyyy/MM/dd") & "' and 
        '" & dtAkhir.Value.ToString("yyyy/MM/dd") & "'"
            Else
                sql = "SELECT tglreturjual as tglpenawaran,  kodereturjual as kodepenawaranjual, tbldepartemen.departemen as kodedepartemen, COALESCE(kodeprojek,'N\A') as kodeprojek,total, diskonrupiah, diskonpersen, totalpajak, biayalain
...
WHERE (kodereturjual ILIKE '%" & eCari.Text & "%' or departemen ILIKE '%" & eCari.Text & "%'  )
        ...
        Try
            ...
            If ringkasan Then
                ...
                ListSat.Columns(1).HeaderText = "Kode Retur"
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        PreviewReturJual.ringkasan = Me.ringkasan
