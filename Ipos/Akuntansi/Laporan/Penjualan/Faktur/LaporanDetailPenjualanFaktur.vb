Public Class LaporanDetailPenjualanFaktur
    ...
    Sub getDataLaporan()
        ...
        If cbSub.SelectedIndex > 0 Then
            sql = "SELECT  tbldetailjual.kodejual as kode,tgljual as tgl,produk, jumlahjual as jumlah,satuan, tbldetailjual.hargajual as harga, jumlahpajak, diskondetailpersen, tblkontak.pelanggan as kontak from tbldetailjual inner join tbljual on tbljual.kodejual = tbldetailjual.kodejual inner join tblharga on tblharga.idharga = tbldetailjual.idharga inner join tblsatuan on tblsatuan.kodesatuan = tblharga.idsatuan inner join tblproduk on tblproduk.idproduk = tblharga.idbarang inner join tblkontak on tblkontak.idpelanggan = tbljual.pelanggan
WHERE (tbljual.kodejual ILIKE '%" & eCari.Text & "%' or tblkontak.pelanggan ILIKE '%" & eCari.Text & "%'  )
