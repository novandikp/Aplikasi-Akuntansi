Public Class LaporanDetailPenjualanFaktur
    ...
    Sub getDataLaporan()
        ...
        If cbSub.SelectedIndex > 0 Then
            sql = "SELECT tbljual.tgljual, tbljual.kodejual, tblkontak.pelanggan, tbldepartemen.departemen, COALESCE(tblprojek.projek,'N/A') as projek, tblgudang.gudang, tbldetailjual.idbarang, tblproduk.produk, tbldetailjual.jumlahjual, tbldetailjual.hargajual from tbldetailjual inner join tbljual on tbljual.kodejual = tbldetailjual.kodejual inner join  tblkontak on tblkontak.idpelanggan = tbljual.pelanggan inner join tbldepartemen on tbldepartemen.iddepartemen = tbljual.kodedepartemen inner join tblgudang on tblgudang.idgudang = tbljual.kodegudang inner join tblproduk on tblproduk.idproduk = tbldetailjual.idbarang left join tblprojek on tblprojek.idprojek  = tbljual.kodeprojek
WHERE (tbljual.kodejual ILIKE '%" & eCari.Text & "%' or departemen ILIKE '%" & eCari.Text & "%'  )
