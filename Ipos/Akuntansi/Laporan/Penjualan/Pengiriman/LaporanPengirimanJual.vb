Public Class LaporanPengirimanJual
    ...
    Sub getDataLaporan()
        If ringkasan Then
            ...
            If cbSub.SelectedIndex > 0 Then
                sql = "SELECT tglpengirimanjual as tglpenawaran,  kodepengirimanjual as kodepenawaranjual, tblkontak.pelanggan as idkontak,total, totalpajak
FROM tblpengirimanjual
INNER join tblkontak
on tblkontak.idpelanggan = tblpengirimanjual.pelanggan
        WHERE (kodepengirimanjual ILIKE '%" & eCari.Text & "%' or tblkontak.pelanggan ILIKE '%" & eCari.Text & "%'  )
...
on tblkontak.idpelanggan = tblpengirimanjual.pelanggan
        ...
        Else
            If cbSub.SelectedIndex > 0 Then
                sql = "SELECT tglpengirimanjual as tglpenawaran, kodepengirimanjual as kodepenawaranjual, tbldepartemen.departemen as kodedepartemen, COALESCE(kodeprojek,'N\A') as kodeprojek,total, diskonrupiah, diskonpersen, totalpajak, biayalain
FROM tblpengirimanjual
INNER join tbldepartemen
on tbldepartemen.iddepartemen = tblpengirimanjual.kodedepartemen
WHERE (kodepengirimanjual ILIKE '%" & eCari.Text & "%' or departemen ILIKE '%" & eCari.Text & "%'  )
          and kodedepartemen ='" & cbSub.SelectedValue & "' and tglpengirimanjual BETWEEN '" & dtAwal.Value.ToString("yyyy/MM/dd") & "' and 
        '" & dtAkhir.Value.ToString("yyyy/MM/dd") & "'"
            Else
                sql = "SELECT tglpengirimanjual as tglpenawaran,  kodepengirimanjual as kodepenawaranjual, tbldepartemen.departemen as kodedepartemen, COALESCE(kodeprojek,'N\A') as kodeprojek,total, diskonrupiah, diskonpersen, totalpajak, biayalain
...
WHERE (kodepengirimanjual ILIKE '%" & eCari.Text & "%' or departemen ILIKE '%" & eCari.Text & "%'  )
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        PreviewPengirimanjual.ringkasan = Me.ringkasan
