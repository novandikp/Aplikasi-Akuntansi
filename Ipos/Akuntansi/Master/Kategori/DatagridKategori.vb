Public Class DatagridKategori
    ...
    Sub fillData()
        ...
        Dim sql As String = "SELECT idkategori, kategori, deskripsikategori from tblkategori where (kategori ilike '%" & eCari.Text & "%' or deskripsikategori ilike '%" & eCari.Text & "%')"
        ...
        Try
            ...
            ListSat.Columns(2).HeaderText = "Deskripsi Kategori"
    ...
    Sub hapusData()
        If Not String.IsNullOrEmpty(idselected) Then
            If dialog("Apakah anda yakin untuk menghapus data ini ?") Then
                If exc("delete from tblkategori where idkategori='" & idselected & "'") Then
    ...
    Sub update()
        Dim sqlupdate As String = "UPDATE public.tblkategori SET kategori=?, deskripsikategori=? where idkategori='" & idselected & "'"
    ...
    Sub insert()
        Dim sqlinsert As String = "INSERT INTO public.tblkategori( kategori, deskripsikategori) 	VALUES (?, ?);"
        Dim isiData As String() = {TBKode.Text, TBNama.Text}
    ...
    Sub setDataSelected()
        Dim sqlselected As String = "SELECT  kategori, deskripsikategori from tblkategori where idkategori='" & idselected & "'"

        TBKode.Text = getValue(sqlselected, "kategori")
        TBNama.Text = getValue(sqlselected, "deskripsikategori")
