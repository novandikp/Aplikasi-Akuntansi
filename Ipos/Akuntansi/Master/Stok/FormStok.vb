Public Class FormStok
    ...
    Private Sub DGStok_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        setGudang()
        focusData()
    ...
    Sub simpanData()
        ...
        Dim selisih As Double = toDouble(jumlahFisik.Text.Replace(",", "."))

        Dim subHPP As Double = selisih * toDouble(unnumberFormat(hpp.Text))

        Dim akunPersediaan As String = "110004"
        Dim sqlinsert As String = "INSERT INTO public.tblstok(
	kodestok, kodegudang, tglstok, kodeakun, idharga, kodedepartemen,  jumlah)
	VALUES (?, ?, ?, ?, ?, ?, ?);"

        Dim kode As String = generateRefrence("SM")
        Dim dataInsert As String() = {kode, cbGudang.SelectedValue, dtTanggal.Value.ToString("yyyy-MM-dd HH:mm:ss").Replace(".", ":"), akunPenerimaan, kodeproduk, cbDepartemen.SelectedValue, jumlahFisik.Text.ToString.Replace(",", ".")}
        If operationQuery(sqlinsert, dataInsert) Then
            If selisih < 0 Then
                ...
                Dim dataDebit As String() = {cbAkun.SelectedValue, "NULL", cbDepartemen.SelectedValue, "NULL", dtTanggal.Value.ToString("yyyy-MM-dd HH:mm:ss").Replace(".", ":"), subHPP.ToString.Replace(",", "."), "0", "ST", kode, "Penambahan Stok , " & cbDepartemen.Text}
                Dim dataKredit As String() = {akunPersediaan, "NULL", cbDepartemen.SelectedValue, "NULL", dtTanggal.Value.ToString("yyyy-MM-dd HH:mm:ss").Replace(".", ":"), "0", subHPP.ToString.Replace(",", "."), "ST", kode, "Penambahan Stok , " & cbDepartemen.Text}
            ...
            Else
                Dim sqlJurnal As String = "INSERT INTO public.tbljurnal(kodeakun, kodeprojek, kodedepartemen, kontak, tgljurnal, debit, kredit, tipe, koderefrensi, deskripsijurnal) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);"
                Dim dataDebit As String() = {akunPersediaan, "NULL", cbDepartemen.SelectedValue, "NULL", dtTanggal.Value.ToString("yyyy-MM-dd HH:mm:ss").Replace(".", ":"), subHPP.ToString.Replace(",", "."), "0", "ST", kode, "Penambahan Stok , " & cbDepartemen.Text}
                Dim dataKredit As String() = {cbAkun.SelectedValue, "NULL", cbDepartemen.SelectedValue, "NULL", dtTanggal.Value.ToString("yyyy-MM-dd HH:mm:ss").Replace(".", ":"), "0", subHPP.ToString.Replace(",", "."), "ST", kode, "Penambahan Stok , " & cbDepartemen.Text}
            ...
            exc("INSERT INTO public.tblstokgudang(idgudang, stok, idharga) VALUES ( '" & cbGudang.SelectedValue & "', " & (toDouble(jumlahBuku.Text.ToString.Replace(",", ".")) + toDouble(jumlahFisik.Text.ToString.Replace(",", "."))).ToString() & " ," & kodeproduk & ") ON CONFLICT  (idgudang,idharga) DO UPDATE  set stok =  EXCLUDED.stok")
    ...
    Private Sub btnSimpan_Click(sender As Object, e As EventArgs) Handles btnSimpan.Click
        ...
        If dialog("Apakah anda yakin untuk menyimpan stok  ini ?") Then
            simpanData()
        End If
    End Sub
