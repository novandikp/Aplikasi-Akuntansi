Public Class DatagridStatus
    ...
    Sub fillData()
        ...
        Dim sql As String = "SELECT idstatus, status, deskripsi from tblstatus where (status ilike '%" & eCari.Text & "%' or deskripsi ilike '%" & eCari.Text & "%')"
        ...
        Try
            ...
            ListSat.Columns(1).HeaderText = "Status"
            ListSat.Columns(2).HeaderText = "Deskripsi Status"
    ...
    Sub hapusData()
        If Not String.IsNullOrEmpty(idselected) Then
            If dialog("Apakah anda yakin untuk menghapus data ini ?") Then
                If exc("delete from tblstatus where idstatus='" & idselected & "'") Then
    ...
    Sub update()
        Dim sqlupdate As String = "UPDATE public.tblstatus SET status=?, deskripsi=? where idstatus='" & idselected & "'"
    ...
    Sub insert()
        Dim sqlinsert As String = "INSERT INTO public.tblstatus( status, deskripsi) 	VALUES (?, ?);"
        Dim isiData As String() = {TBKode.Text, TBNama.Text}
    ...
    Sub setDataSelected()
        Dim sqlselected As String = "SELECT  status, deskripsi from tblstatus where idstatus='" & idselected & "'"

        TBKode.Text = getValue(sqlselected, "status")
        TBNama.Text = getValue(sqlselected, "deskripsi")
