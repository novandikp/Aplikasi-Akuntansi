Public Class DatagridKategoriaset
    ...
    Sub fillData()
        ...
        Dim sql As String = "SELECT idkategoriaset, kategoriaset, metodepenyusutan, penggunaan
	FROM public.tblkategoriaset where (kategoriaset ilike '%" & eCari.Text & "%' )"
        ...
        Try
            ...
            ListSat.Columns(1).HeaderText = "Kategori"
            ListSat.Columns(2).HeaderText = "Metode Penyusutan"
            ListSat.Columns(3).HeaderText = "Penggunaan (Tahun)"
    ...
    Sub hapusData()
        If Not String.IsNullOrEmpty(idselected) Then
            If dialog("Apakah anda yakin untuk menghapus data ini ?") Then
                If exc("delete from tblkategoriaset where idkategoriaset='" & idselected & "'") Then
    ...
    Sub simpanData()
        ...
        ElseIf Not Double.TryParse(TBNama.Text, 0) Then
            dialogError("Masukkan format yang benar")
        Else
    ...
    Sub update()
        Dim sqlupdate As String = "UPDATE public.tblkategoriaset SET kategoriaset=?, metodepenyusutan=?,penggunaan=? where idkategoriaset='" & idselected & "'"
        Dim isiData As String() = {TBKode.Text, MetroComboBox1.Text, TBNama.Text.Replace(",", ".")}
    ...
    Sub insert()
        Dim sqlinsert As String = "INSERT INTO public.tblkategoriaset( kategoriaset, metodepenyusutan,penggunaan) 	VALUES (?, ?,?);"
        Dim isiData As String() = {TBKode.Text, MetroComboBox1.Text, TBNama.Text.Replace(",", ".")}
    ...
    Sub setDataSelected()
        Dim sqlselected As String = "SELECT idkategoriaset, kategoriaset, metodepenyusutan, penggunaan from tblkategoriaset where idkategoriaset='" & idselected & "'"

        TBKode.Text = getValue(sqlselected, "kategoriaset")
        TBNama.Text = getValue(sqlselected, "penggunaan")
        MetroComboBox1.Text = getValue(sqlselected, "metodepenyusutan")
