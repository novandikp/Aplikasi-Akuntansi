...
Public Class FormPenawaranJual
    ...
    Sub closeTab()
        Dim listItemForms As FormPenawaranJual = Application.OpenForms("FormPenjualan")
        Me.Close()
    End Sub
    ...
    Sub continueOrder()
        Dim sqlorder As String = "select kodepenawaranjual, tglpenawaranjual, kodedepartemen, kodeprojek, total, diskonrupiah, diskonpersen, totalpajak, biayalain, kasbiayalain, kasdiskon, nomerdokumen, tgldokumen, pelanggan, kodegudang from tblpenawaranjual where kodepenawaranjual='" & TBnotransaksi.Text & "'"
        Dim sqldetail As String = "select tbldetailpenawaranjual.idharga,idproduk,diskondetailpersen,produk, idbarang,jumlahjual, tbldetailpenawaranjual.hargajual,satuan,jumlahpajak from  tbldetailpenawaranjual  inner join tblharga on tblharga.idharga = tbldetailpenawaranjual.idharga inner join tblproduk on tblproduk.idproduk = tblharga.idbarang inner join tblsatuan on tblsatuan.kodesatuan = tblharga.idsatuan where kodepenawaranjual='" & TBnotransaksi.Text & "'"
        saved = True
        dgKeranjang.Rows.Clear()
    ...
    Sub simpanBayar(bayar As DialogTransaksiTanpaBayar)
        ...
        If Not edited Then
            ...
            Dim sql As String = "INSERT INTO public.tblpenawaranjual(kodepenawaranjual, tglpenawaranjual, kodedepartemen, kodeprojek, total, diskonrupiah, diskonpersen, totalpajak, biayalain, kasbiayalain, kasdiskon, nomerdokumen, tgldokumen, pelanggan, kodegudang, statuspenawaran) 	VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);"
        ...
        Else
            ...
            Dim sql As String = "UPDATE public.tblpenawaranjual SET kodepenawaranjual=?, tglpenawaranjual=?, kodedepartemen=?, kodeprojek=?, total=?, diskonrupiah=?, diskonpersen=?, totalpajak=?, biayalain=?, kasbiayalain=?, kasdiskon=?, nomerdokumen=?, tgldokumen=?, pelanggan=?, kodegudang=? where kodepenawaranjual=?;DELETE FROM tbldetailpenawaranjual where kodepenawaranjual=?"
            If operationQuery(sql, isidata) Then
                isidetail(TBnotransaksi.Text)
                dialogSukses("Berhasil")
                edited = False
