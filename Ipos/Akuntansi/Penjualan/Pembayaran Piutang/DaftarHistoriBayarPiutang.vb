Public Class DaftarHistoriBayarPiutang
    ...
    Sub fillData()
        ...
        Dim sql As String = "select tglbayarpiutang,kodebayarpiutang,tblkontak.pelanggan, tblbayarpiutang.kodejual,tblbayarpiutang.bayarpiutang+tblbayarpiutang.biayalain as nilai from tblbayarpiutang inner join tbljual on tbljual.kodejual = tblbayarpiutang.kodejual inner join tblkontak on tblkontak.idpelanggan = tbljual.pelanggan "
        Dim filter As String = "WHERE (kodebayarpiutang ilike '%" & cari & "%' or tblkontak.pelanggan ilike '%" & cari & "%') AND tglbayarpiutang BETWEEN '" & tglAwal & "' AND '" & tglAkhir & "' order by tglbayarpiutang,kodebayarpiutang"
        ...
        If ListSat.SelectedRows.Count = 1 Then
            ...
            If True Then
                If dialog("Apakah anda yakin untuk menghapus data ini ?") Then
                    Dim sqlhapus = "DELETE FROM tblbayarpiutang where kodebayarpiutang = '" & idselected & "';"
