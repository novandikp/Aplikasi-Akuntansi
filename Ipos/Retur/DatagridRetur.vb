Public Class DatagridRetur
    ...
    Sub getDataPelanggan()
        ...
        Me.ListSat.DataSource =
            getData(
                "SELECT iddetailjual, fakturjual, produk, jumlahjual, returjual, idjual from view_detailjual where tgljual between ('" &
                awal & " 00:00:00') and ('" & akhir & " 23:59:59') and flag=1 and idkategori > 1 and cabang=" & Form1.idcabang &
                " and (produk LIKE '%" & eCari.Text & "%' OR fakturjual LIKE '%" & eCari.Text & "%')")
        makeFillDG(ListSat)
        Try
    ...
    Private Sub ListItem_KeyDown(sender As Object, e As KeyEventArgs) _
        Handles Me.KeyDown, eCari.KeyDown, ListSat.KeyDown, AddBtn.KeyDown, DTAkhir.KeyDown, DTAwal.KeyDown,
                Button2.KeyDown, AddBtn.KeyDown, Button3.KeyDown, Button1.KeyDown
    ...
    Sub closeTab()
        ...
        Dim listItemForms As DatagridRetur = Application.OpenForms("DatagridRetur")
        Me.Close()
        listItemForms.Close()
        Return
    ...
    Sub tambahPelanggan()
        If ListSat.SelectedRows.Count = 1 Then
            ...
            Else
                DialogRetur.iddetailjual = ListSat.Rows(ListSat.SelectedRows(0).Index).Cells(0).Value.ToString
                DialogRetur.ShowDialog()
            End If
        Else
            dialogError("Pilih item terlebih dahulu")
        End If
    ...
    Sub detail()
        If ListSat.SelectedRows.Count = 1 Then
            ...
            HistoriRetur.idjual = beli
            HistoriRetur.getDataPelanggan()
            HistoriRetur.ShowDialog()
        Else
            dialogError("Pilih item terlebih dahulu")
        End If
        ListSat.ClearSelection()
