Public Class HistoryReturBeli
    ...
    Function checkStok(idbarang As String) As Boolean
        Dim sql =
                "SELECT jumlahreturbeli * nilaijual/jumlahjual as retur, barcode from view_returjual where idreturjual=" &
                idbarang
        Dim stoksaatini =
                Double.Parse(
                    getValue(
    ...
    Sub getDataPelanggan()
        ...
        Me.ListSat.DataSource =
            getData(
                "SELECT idreturbeli ,produk, fakturbeli, tglreturbeli, jumlahreturbeli from view_returbeli where tglreturbeli between ('" &
                awal & " 00:00:00') and ('" & akhir & " 23:59:59')  and (produk LIKE '%" & eCari.Text &
                "%' OR fakturbeli LIKE '%" & eCari.Text & "%') and idbeli=" & idbeli)
        makeFillDG(ListSat)
        Try
    ...
    Sub tambahPelanggan()
        If ListSat.SelectedRows.Count = 1 Then
            If dialog("Apakah anda yakin untuk menghapus data") Then
                ...
                Dim sql = "select * from view_returbeli where idreturbeli=" & id
                If True Then
                    Dim produk = getValue(sql, "barcode")
                    exc(
                        "update tbldetailbeli set returbeli = returbeli - '" & getValue(sql, "jumlahreturbeli") &
                        "' where iddetailbeli=" & getValue(sql, "iddetailbeli"))
                    Dim nilai As Double =
                            Double.Parse(
                                getValue(
                                    "SELECT jumlahreturbeli * nilaibeli/jumlahbeli as retur, barcode from view_returbeli where idreturbeli=" &
                                    id, "retur"))
                    Dim datastok As String() = New String() _
                            {produk, getValue(sql, "jumlahreturbeli").ToString.Replace(",", "."),
                             getValue(sql, "satuanjual"), nilai.ToString.Replace(",", "."), Date.Now.ToString(),
                             "SALAH RETUR BELI" & faktur, "0"}
                    Dim querystok =
                    ...
                    exc("delete from tblreturbeli where idreturbeli =" & id)
    ...
    Private Sub HistoryReturBeli_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        DatagridReturBeli.getDataPelanggan()
    End Sub
End Class