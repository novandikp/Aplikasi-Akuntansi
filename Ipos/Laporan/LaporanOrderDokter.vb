Public Class LaporanOrderDokter
    ...
    Sub laporanBarangLengkap()
        ...
        sql =
            "SELECT iddetailjual, idjual,tgljual, produk, pelanggan, jumlahjual, satuanjual, hargajual, jumlahjual*hargajual as sub from view_detailjual  where tgljual between ('" &
            awal & " 00:00:00') and ('" & akhir & " 23:59:59') and idkategori = 1 and (produk like '%" & eCari.Text &
            "%' or pelanggan like '%" & eCari.Text & "%' or fakturjual like '%" & eCari.Text & "%')"
        ...
        Try
            ...
            DataGridView1.Columns(4).HeaderText = "Pelanggan"
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        If open Then
            open = False
            Dim preview As PreviewPenjualan = New PreviewPenjualan()
            If ComboBox1.SelectedIndex = 0 Then
                preview.group = "pelanggan"
            Else
                preview.group = "produk"
            End If
            Dim awal = DTawal.Value.ToString("yyyy/MM/dd")
            Dim akhir = DtAkhir.Value.ToString("yyyy/MM/dd")
            preview.sql = "SELECT * from view_detailjual  where tgljual between ('" &
                          awal & " 00:00:00') and ('" & akhir & " 23:59:59') and idkategori = 1 and (pelanggan like '%" & eCari.Text &
                          "%' or fakturjual like '%" & eCari.Text & "%')"
            preview.Text = "Laporan Service Teknisi"
            preview.servis = True
