...
Public Class DialogBayar
    ...
    Sub simpanBayar()
        ...
        ElseIf Not cbMetode.SelectedIndex = 0 And String.IsNullOrWhiteSpace(TBNoAkun.Text) Then
            dialogError("Isi nomer akun dengan benar")
        ...
        ElseIf (Not hutang And kembali >= 0) Or (hutang And kembali < 0) Then
            Dim metode = "Tunai " & cbMetode.Text
            Dim akun = cbAkun.Text
            Dim noakun = TBNoAkun.Text

            If hutang Then
                metode = "Hutang " & cbMetode.Text
            End If
            ...
            If operationQuery(sql, isidata) Then
                ...
                End If
                Dim idbeli As String = getValue("SELECT idjual from tbljual order by idjual desc", "idjual")
                ...
                If dialog("Apakah anda ingin mencetak struk ?") Then
                    ...
                    If struk = "58 mm" Then
                        ...
                        Struk5.Reset()
                        Struk5.LocalReport.ReportEmbeddedResource = "Ipos.Struk58.rdlc"
                        Dim sqlidentitasa As String =
                                "select idtoko, namatoko, alamatoko, notoko, caption1, caption2, caption3, printstruk, logo, kota from tblidentitas where idtoko=1"

                        Dim sqla As String = "SELECT * from view_detailjual where idjual = " & idbeli
                        Struk5.LocalReport.DataSources.Clear()
                        Struk5.LocalReport.DataSources.Add(New ReportDataSource("DataSet1",
                                                                                CType(getData(sqla), DataTable)))
                        Struk5.SetDisplayMode(DisplayMode.PrintLayout)

                        Struk5.LocalReport.SetParameters(New ReportParameter("namaToko",
                                                                             getValue(sqlidentitasa, "namatoko").
                                                                                ToString))
                        Struk5.LocalReport.SetParameters(New ReportParameter("alamatToko",
                                                                             getValue(sqlidentitasa, "alamatoko").
                                                                                ToString))
                        Struk5.LocalReport.SetParameters(New ReportParameter("nomorToko",
                                                                             getValue(sqlidentitasa, "notoko").ToString))
                        Struk5.LocalReport.SetParameters(New ReportParameter("kota",
                                                                             getValue(sqlidentitasa, "kota").ToString))
                        Struk5.LocalReport.SetParameters(New ReportParameter("username", Form1.username))
                        Struk5.LocalReport.SetParameters(New ReportParameter("caption1",
                                                                             getValue(sqlidentitasa, "caption1").
                                                                                ToString))
                        Struk5.LocalReport.SetParameters(New ReportParameter("caption2",
                                                                             getValue(sqlidentitasa, "caption2").
                                                                                ToString))
                        Struk5.LocalReport.SetParameters(New ReportParameter("caption3",
                                                                             getValue(sqlidentitasa, "caption3").
                                                                                ToString))

                        Struk5.RefreshReport()


                    ElseIf struk = "70 mm" Then
                        Struk7.Reset()
                        Struk7.LocalReport.ReportEmbeddedResource = "Ipos.Struk70.rdlc"
                        Dim sqlidentitasa As String =
                                "select idtoko, namatoko, alamatoko, notoko, caption1, caption2, caption3, printstruk, logo, kota from tblidentitas where idtoko=1"

                        Dim sqla As String = "SELECT * from view_detailjual where idjual = " & idbeli
                        Struk7.LocalReport.DataSources.Clear()
                        Struk7.LocalReport.DataSources.Add(New ReportDataSource("DataSet1",
                                                                                CType(getData(sqla), DataTable)))
                        Struk7.SetDisplayMode(DisplayMode.PrintLayout)

                        Struk7.LocalReport.SetParameters(New ReportParameter("namaToko",
                                                                             getValue(sqlidentitasa, "namatoko").
                                                                                ToString))
                        Struk7.LocalReport.SetParameters(New ReportParameter("alamatToko",
                                                                             getValue(sqlidentitasa, "alamatoko").
                                                                                ToString))
                        Struk7.LocalReport.SetParameters(New ReportParameter("nomorToko",
                                                                             getValue(sqlidentitasa, "notoko").ToString))
                        Struk7.LocalReport.SetParameters(New ReportParameter("kota",
                                                                             getValue(sqlidentitasa, "kota").ToString))
                        Struk7.LocalReport.SetParameters(New ReportParameter("username", Form1.username))
                        Struk7.LocalReport.SetParameters(New ReportParameter("caption1",
                                                                             getValue(sqlidentitasa, "caption1").
                                                                                ToString))
                        Struk7.LocalReport.SetParameters(New ReportParameter("caption2",
                                                                             getValue(sqlidentitasa, "caption2").
                                                                                ToString))
                        Struk7.LocalReport.SetParameters(New ReportParameter("caption3",
                                                                             getValue(sqlidentitasa, "caption3").
                                                                                ToString))

                        Struk7.RefreshReport()


                    Else
                        StrukA.Reset()
                        StrukA.LocalReport.ReportEmbeddedResource = "Ipos.StrukA5.rdlc"
                        Dim sqlidentitasa As String =
                                "select idtoko, namatoko, alamatoko, notoko, caption1, caption2, caption3, printstruk, logo, kota from tblidentitas where idtoko=1"

                        Dim sqla As String = "SELECT * from view_detailjual where idjual = " & idbeli
                        StrukA.LocalReport.DataSources.Clear()
                        StrukA.LocalReport.DataSources.Add(New ReportDataSource("DataSet1",
                                                                                CType(getData(sqla), DataTable)))
        ...
        ElseIf (Not hutang And kembali >= 0) Or (hutang And kembali < 0) Then
            ...
            Dim isidata As String() = New String() _
                    {fakturbeli, tanggal, total.ToString, unnumberFormat(tbBayar.Text), kembali.ToString, supplier,
                     metode, akun, noakun, TBDiskon.Text, user, dokter, resep}
            Dim sql As String =
    ...
    Private Sub Button5_KeyDown(sender As Object, e As KeyEventArgs) _
        Handles tbTotal.KeyDown, tbKembali.KeyDown, tbBayar.KeyDown, Button6.KeyDown, Button5.KeyDown, Button4.KeyDown,
                Button3.KeyDown, Button2.KeyDown, Button1.KeyDown, TBDiskon.KeyDown, TBNoAkun.KeyDown, cbMetode.KeyDown,
                cbAkun.KeyDown, Button7.KeyDown, Button8.KeyDown, Button9.KeyDown
    ...
    Sub isiBank()
        cbAkun.Items.Clear()
        cbAkun.Items.Add("BCA")
        cbAkun.Items.Add("BRI")
        cbAkun.Items.Add("Mandiri")
        cbAkun.Items.Add("BNI")
        cbAkun.Items.Add("CIMB NIAGA")
        cbAkun.Items.Add("LAIN-LAIN")
        cbAkun.SelectedIndex = 0
    End Sub

    Sub isiEmoney()
        cbAkun.Items.Clear()
        cbAkun.Items.Add("GOPAY")
        cbAkun.Items.Add("OVO")
        cbAkun.Items.Add("LINK AJA")
        cbAkun.Items.Add("LAIN-LAIN")
        cbAkun.SelectedIndex = 0
    End Sub

    Private Sub cbMetode_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbMetode.SelectedIndexChanged
        TBNoAkun.Text = ""
        If cbMetode.SelectedIndex = 0 Then
            panelNonCash.Visible = False
        ElseIf cbMetode.SelectedIndex = 1 Then
